...
| :--- | ---: | :--- | :--- | :--- |
| `0x00` | 24 | `Xilinx WAVE DATABASE 01`, NUL terminated | hex dump of any database | all 999 cases |
| `0x18` | 24 | `Xilinx Simulator`, NUL terminated | same | all 999 cases |
| `0x30` | 8 | `uint64` `0x40` | same | constant in all 999 cases |
| `0x38` | 4 | `uint32` Unix time the database was written | the noise mask, two runs of `t3_tr1` | decodes to the file's own mtime |
| `0x48` | 24 | three `uint64` file offsets of directory entries | following the values: each lands on a NUL terminated section name | all 999 cases |
| `0x98` | 12 | three `uint32` `0x30` | hex dump | constant in all 999 cases |
| `0xc0` | 4 | `uint32` `3` | hex dump | constant in all 999 cases |
...
*Confirmed by* the reader's check across all 999 cases, including
...
| Offset | Len | Content | Found by | Confirmed by |
| :--- | ---: | :--- | :--- | :--- |
| `0x00` | 8 | `uint64` simulation end time, in the time unit the DBG section names, picoseconds in every case before tier 21 | the correlation sweep across all cases | 999 of 999 against `end_time_ns` in `truth.json`; `t21_v_ts_1ns_1ns` holds 100 for 100 ns |
| `0x08` | 4 | `uint32` `0x3e9` | hex dump | constant in all 999 cases |
| `0x0c` | 4 | `uint32` number of arena table slots | recorded as the constant `3` until the tier 7 sweep over every fixed word; it is 4, 5 and 6 in the signal count cases | the reader checks it against the table length in 999 of 999 cases |
| `0x10` | 8 | `uint64` `0x800` | hex dump | constant in all 999 cases; the arena span, by its value |
| `0x18` | 8 | `uint64` handle space, the bytes of handle space the objects occupy | comparing cases | the arena table rule above, 999 of 999 cases |
| `0x20` | 4 | `uint32` `0xc8` | hex dump | constant in all 999 cases; the arena table's offset, by its value |
| `0x24` | 4 | `uint32` `0` | hex dump | constant in all 999 cases |
| `0x28` | 8 | `uint64` `0` | hex dump | constant in all 999 cases |
| `0x30` | 8 | `uint64` number of logged ranges at the marker, `0` in `t0_nosig` | read as a flag until `t9_port_rec` held `2` | 999 of 999 cases, checked against the marker |
...
| `0x44` | 4 | `uint32` `0x64` | hex dump | constant in all 999 cases |
