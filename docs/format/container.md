...
| :--- | ---: | :--- | :--- | :--- |
| `0x00` | 24 | `Xilinx WAVE DATABASE 01`, NUL terminated | hex dump of any database | all 1061 cases |
| `0x18` | 24 | `Xilinx Simulator`, NUL terminated | same | all 1061 cases |
| `0x30` | 8 | `uint64` `0x40` | same | constant in all 1061 cases |
| `0x38` | 4 | `uint32` Unix time the database was written | the noise mask, two runs of `t3_tr1` | decodes to the file's own mtime |
| `0x48` | 24 | three `uint64` file offsets of directory entries | following the values: each lands on a NUL terminated section name | all 1061 cases |
| `0x98` | 12 | three `uint32` `0x30` | hex dump | constant in all 1061 cases |
| `0xc0` | 4 | `uint32` `3` | hex dump | constant in all 1061 cases |
...
Every slot is `0` when the script logs nothing: `t57_log_none____`
declares eleven objects over three arenas, has four slots of `0`, a
marker count and offset of `0` as `t0_nosig` has, no page, and the
directory right after the debug section, 4007 bytes in all.
The same file comes out of a script that names only a variable, a bit
of a vector, a record field or a generate statement, none of which
`log_wave` logs, see [values.md](values.md).
...
*Confirmed by* the reader's check across all 1061 cases, including
...
The first record, at `0xfa7` in a file of this size, goes to whichever
arena is written first: slot 2 alone holds `0xfa7` in
`t57_log_con_____`, where only the constant in arena 2 is logged, slot
1 alone in `t57_log_rec_____`, and slots 1 and 2 hold `0xfa7 0x1467`
in `t57_log_gen_it__`.
...
| Offset | Len | Content | Found by | Confirmed by |
| :--- | ---: | :--- | :--- | :--- |
| `0x00` | 8 | `uint64` simulation end time, in the time unit the DBG section names, picoseconds in every case before tier 21 | the correlation sweep across all cases | 1061 of 1061 against `end_time_ns` in `truth.json`; `t21_v_ts_1ns_1ns` holds 100 for 100 ns |
| `0x08` | 4 | `uint32` `0x3e9` | hex dump | constant in all 1061 cases |
| `0x0c` | 4 | `uint32` number of arena table slots | recorded as the constant `3` until the tier 7 sweep over every fixed word; it is 4, 5 and 6 in the signal count cases | the reader checks it against the table length in 1061 of 1061 cases |
| `0x10` | 8 | `uint64` `0x800` | hex dump | constant in all 1061 cases; the arena span, by its value |
| `0x18` | 8 | `uint64` handle space, the bytes of handle space the objects occupy | comparing cases | the arena table rule above, 1061 of 1061 cases |
| `0x20` | 4 | `uint32` `0xc8` | hex dump | constant in all 1061 cases; the arena table's offset, by its value |
| `0x24` | 4 | `uint32` `0` | hex dump | constant in all 1061 cases |
| `0x28` | 8 | `uint64` `0` | hex dump | constant in all 1061 cases |
| `0x30` | 8 | `uint64` number of logged ranges at the marker, `0` in `t0_nosig` | read as a flag until `t9_port_rec` held `2` | 1061 of 1061 cases, checked against the marker |
...
| `0x44` | 4 | `uint32` `0x64` | hex dump | constant in all 1061 cases |
