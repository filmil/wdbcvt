...
| Case | The package declares | Handle space |
| :--- | :--- | ---: |
| `t77_pkc_1type___` | one type, no object | `0x1250` |
| `t77_pkc_1fn_____` | one function | `0x1250` |
| `t77_pkc_4fn_____` | four functions | `0x1250` |
| `t77_pkc_1con____` | one integer constant | `0x1258` |
| `t77_pkc_1real___` | one real constant | `0x1258` |
| `t77_pkc_4con____` | four integer constants | `0x1260` |
...
Every first signal sits at `0x768`, and what the `0x738` below it
holds is part of question 6.
Subprogram frame objects are the only ones seen down there, so this
tier fills a frame and watches the signal.

| Case | The procedure declares | Frame runs | `s` | Handle space |
| :--- | :--- | :--- | ---: | ---: |
| `t79_frm_001int__` | one integer variable | `0xd0` to `0xd4` | `0x768` | `0x11d0` |
| `t79_frm_016int__` | sixteen | `0xd0` to `0x110` | `0x768` | `0x11d0` |
| `t79_frm_064int__` | sixty four | `0xd0` to `0x1d0` | `0x768` | `0x11d0` |
| `t79_frm_256int__` | two hundred and fifty six | `0xd0` to `0x4d0` | `0x768` | `0x11d0` |
| `t79_frm_512int__` | five hundred and twelve | `0xd0` to `0x8d0` | `0x768` | `0x11d0` |
| `t79_frm_two_64__` | two procedures, sixty four each | `0xd0` to `0x1d0` | `0x768` | `0x11d0` |

The frame of 512 integers runs past the signal's own handle and
nothing moves, and the two procedures number their frames from the
same `0xd0`.
So a frame handle is an offset in the subprogram's own numbering
...
**Tier 82: what a package body adds.**
The library packages leave more than that flat `0x50`, and the tier 81
packages had nothing a library package has.
This tier puts those things in: constants in the body, a deferred
constant, and a static value inside the package's own function.

| Case | The package has | Handle space | `k` |
| :--- | :--- | ---: | ---: |
| `t82_pkb_none____` | a function and an empty body | `0x1250` | `0xe08` |
| `t82_pkb_1con____` | one integer constant in the body | `0x1258` | `0xe10` |
| `t82_pkb_4con____` | four of them | `0x1260` | `0xe18` |
| `t82_pkb_arr16___` | a constant array of sixteen in the body | `0x12a0` | `0xe58` |
| `t82_pkb_deferred` | a deferred constant, valued in the body | `0x1258` | `0xe10` |
| `t82_pkb_fn_stat_` | a function with an array local of a literal value | `0x1260` | `0xe08` |

A constant of the body moves `k`, so it lies in the block like a
constant of the header, and a deferred constant does the same.
The function's static value does not move `k` and adds `0x10`, its own
bytes, past the second region.
