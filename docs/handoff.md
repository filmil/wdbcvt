...
The reader, `wdbcvt`, opens every one of the 1097 corpus cases of tiers
1 to 72 and reproduces their `truth.json` and Vivado's own VCD.
...
The generators of tiers 57 to 72 are in `tools/corpus/`, see its
README; a new tier starts by copying `gen_t72/main.go`.
The registration anchor for tier 73 in `hdl/corpus/BUILD.bazel` is the
last tier 72 case in sorted order, `t72_dbg_wave____`.
...
3. Document in the `format/` page for the area, then `format.md`:
   ...
   comparison rows after the tier 72 rows, guesses in the open
   questions; a tier section in `corpus.md` before "Record which
   comparison produced which finding"; and the count 1097 upward
