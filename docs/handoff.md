...
The reader, `wdbcvt`, opens every one of the 1127 corpus cases of tiers
1 to 77 and reproduces their `truth.json` and Vivado's own VCD.
...
The generators of tiers 57 to 77 are in `tools/corpus/`, see its
README; a new tier starts by copying `gen_t77/main.go`.
The registration anchor for tier 78 in `hdl/corpus/BUILD.bazel` is the
last tier 77 case in sorted order, `t77_pkc_arr16___`.
...
3. Document in the `format/` page for the area, then `format.md`:
   ...
   comparison rows after the tier 77 rows, guesses in the open
   questions; a tier section in `corpus.md` before "Record which
   comparison produced which finding"; and the count 1127 upward
