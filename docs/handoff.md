...
The reader, `wdbcvt`, opens every one of the 1163 corpus cases of tiers
1 to 82 and reproduces their `truth.json` and Vivado's own VCD.
...
The generators of tiers 57 to 82 are in `tools/corpus/`, see its
README; a new tier starts by copying `gen_t82/main.go`.
The registration anchor for tier 83 in `hdl/corpus/BUILD.bazel` is the
last tier 82 case in sorted order, `t82_pkb_none____`.
...
3. Document in the `format/` page for the area, then `format.md`:
   ...
   comparison rows after the tier 82 rows, guesses in the open
   questions; a tier section in `corpus.md` before "Record which
   comparison produced which finding"; and the count 1163 upward
