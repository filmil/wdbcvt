...
The reader, `wdbcvt`, opens every one of the 1090 corpus cases of tiers
1 to 71 and reproduces their `truth.json` and Vivado's own VCD.
...
The generators of tiers 57 to 71 are in `tools/corpus/`, see its
README; a new tier starts by copying `gen_t71/main.go`.
The registration anchor for tier 72 in `hdl/corpus/BUILD.bazel` is the
last tier 71 case in sorted order, `t71_rlw_vhdl_gen`.
...
3. Document in the `format/` page for the area, then `format.md`:
   ...
   comparison rows after the tier 71 rows, guesses in the open
   questions; a tier section in `corpus.md` before "Record which
   comparison produced which finding"; and the count 1090 upward
