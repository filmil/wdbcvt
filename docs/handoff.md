...
The reader, `wdbcvt`, opens every one of the 1107 corpus cases of tiers
1 to 74 and reproduces their `truth.json` and Vivado's own VCD.
...
The generators of tiers 57 to 74 are in `tools/corpus/`, see its
README; a new tier starts by copying `gen_t74/main.go`.
The registration anchor for tier 75 in `hdl/corpus/BUILD.bazel` is the
last tier 74 case in sorted order, `t74_lgw_star____`.
...
3. Document in the `format/` page for the area, then `format.md`:
   ...
   comparison rows after the tier 74 rows, guesses in the open
   questions; a tier section in `corpus.md` before "Record which
   comparison produced which finding"; and the count 1107 upward
