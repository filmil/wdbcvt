...
The reader, `wdbcvt`, opens every one of the 1119 corpus cases of tiers
1 to 76 and reproduces their `truth.json` and Vivado's own VCD.
...
The generators of tiers 57 to 76 are in `tools/corpus/`, see its
README; a new tier starts by copying `gen_t76/main.go`.
The registration anchor for tier 77 in `hdl/corpus/BUILD.bazel` is the
last tier 76 case in sorted order, `t76_stc_sv_str__`.
...
3. Document in the `format/` page for the area, then `format.md`:
   ...
   comparison rows after the tier 76 rows, guesses in the open
   questions; a tier section in `corpus.md` before "Record which
   comparison produced which finding"; and the count 1119 upward
