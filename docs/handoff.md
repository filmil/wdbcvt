...
The reader, `wdbcvt`, opens every one of the 1157 corpus cases of tiers
1 to 81 and reproduces their `truth.json` and Vivado's own VCD.
...
The generators of tiers 57 to 81 are in `tools/corpus/`, see its
README; a new tier starts by copying `gen_t81/main.go`.
The registration anchor for tier 82 in `hdl/corpus/BUILD.bazel` is the
last tier 81 case in sorted order, `t81_pkt_two_pk__`.
...
3. Document in the `format/` page for the area, then `format.md`:
   ...
   comparison rows after the tier 81 rows, guesses in the open
   questions; a tier section in `corpus.md` before "Record which
   comparison produced which finding"; and the count 1157 upward
