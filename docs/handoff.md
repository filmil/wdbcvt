...
The reader, `wdbcvt`, opens every one of the 1101 corpus cases of tiers
1 to 73 and reproduces their `truth.json` and Vivado's own VCD.
...
The generators of tiers 57 to 73 are in `tools/corpus/`, see its
README; a new tier starts by copying `gen_t73/main.go`.
The registration anchor for tier 74 in `hdl/corpus/BUILD.bazel` is the
last tier 73 case in sorted order, `t73_prt_inst_lst`.
...
3. Document in the `format/` page for the area, then `format.md`:
   ...
   comparison rows after the tier 73 rows, guesses in the open
   questions; a tier section in `corpus.md` before "Record which
   comparison produced which finding"; and the count 1101 upward
