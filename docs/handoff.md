...
The reader, `wdbcvt`, opens every one of the 1112 corpus cases of tiers
1 to 75 and reproduces their `truth.json` and Vivado's own VCD.
...
The generators of tiers 57 to 75 are in `tools/corpus/`, see its
README; a new tier starts by copying `gen_t75/main.go`.
The registration anchor for tier 76 in `hdl/corpus/BUILD.bazel` is the
last tier 75 case in sorted order, `t75_fil_rec_____`.
...
3. Document in the `format/` page for the area, then `format.md`:
   ...
   comparison rows after the tier 75 rows, guesses in the open
   questions; a tier section in `corpus.md` before "Record which
   comparison produced which finding"; and the count 1112 upward
