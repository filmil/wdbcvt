...
The reader, `wdbcvt`, opens every one of the 1061 corpus cases of tiers
1 to 68 and reproduces their `truth.json` and Vivado's own VCD.
...
The generators of tiers 57 to 68 are in `tools/corpus/`, see its
README; a new tier starts by copying `gen_t68/main.go`.
The registration anchor for tier 69 in `hdl/corpus/BUILD.bazel` is the
last tier 68 case in sorted order, `t68_str_noinit__`.
...
3. Document in the `format/` page for the area, then `format.md`:
   ...
   comparison rows after the tier 68 rows, guesses in the open
   questions; a tier section in `corpus.md` before "Record which
   comparison produced which finding"; and the count 1061 upward
