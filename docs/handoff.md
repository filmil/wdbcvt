...
The reader, `wdbcvt`, opens every one of the 1140 corpus cases of tiers
1 to 78 and reproduces their `truth.json` and Vivado's own VCD.
...
The generators of tiers 57 to 78 are in `tools/corpus/`, see its
README; a new tier starts by copying `gen_t78/main.go`.
The registration anchor for tier 79 in `hdl/corpus/BUILD.bazel` is the
last tier 78 case in sorted order, `t78_scp_solo3___`.
...
3. Document in the `format/` page for the area, then `format.md`:
   ...
   comparison rows after the tier 78 rows, guesses in the open
   questions; a tier section in `corpus.md` before "Record which
   comparison produced which finding"; and the count 1140 upward
