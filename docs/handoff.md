...
The reader, `wdbcvt`, opens every one of the 1082 corpus cases of tiers
1 to 70 and reproduces their `truth.json` and Vivado's own VCD.
...
The generators of tiers 57 to 70 are in `tools/corpus/`, see its
README; a new tier starts by copying `gen_t70/main.go`.
The registration anchor for tier 71 in `hdl/corpus/BUILD.bazel` is the
last tier 70 case in sorted order, `t70_num_d_then_q`.
...
3. Document in the `format/` page for the area, then `format.md`:
   ...
   comparison rows after the tier 70 rows, guesses in the open
   questions; a tier section in `corpus.md` before "Record which
   comparison produced which finding"; and the count 1082 upward
