...
The reader, `wdbcvt`, opens every one of the 1074 corpus cases of tiers
1 to 69 and reproduces their `truth.json` and Vivado's own VCD.
...
The generators of tiers 57 to 69 are in `tools/corpus/`, see its
README; a new tier starts by copying `gen_t69/main.go`.
The registration anchor for tier 70 in `hdl/corpus/BUILD.bazel` is the
last tier 69 case in sorted order, `t69_vcl_wire_ini`.
...
3. Document in the `format/` page for the area, then `format.md`:
   ...
   comparison rows after the tier 69 rows, guesses in the open
   questions; a tier section in `corpus.md` before "Record which
   comparison produced which finding"; and the count 1074 upward
