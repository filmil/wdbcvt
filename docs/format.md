...
instance record per logged object, and each record holds the object's
...
against the `truth.json` of all 1172 cases, and against the `sim.vcd`
...
| :--- | :--- | :--- | :--- |
| Magic `Xilinx WAVE DATABASE 01`, producer `Xilinx Simulator` | hex dump of `t1_bit_one_edge` | all 1172 cases | container |
| `0x38` is a Unix timestamp | noise mask, two runs of `t3_tr1` | equals the file mtime | container |
| `0x48` holds three pointers to 48 byte directory entries | `strings -t d` on `t2_flat3`, then reading the values | all 1172 cases, each pointer lands on a name | container |
...
| The arena table has `ceil(handle space / 0x800)` slots | `t7_sig07` broke the `ceil(objects / 4) + 1` guess | the reader checks it in 1172 of 1172 | container |
| Trailer `+0x0c` is the arena table slot count | sweep of every fixed word over the 63 cases of tier 7 | 1172 of 1172, checked by the reader | container |
| Trailer `+0x18` is the handle space size | `t7_sig07` to `t7_sig24` against the slot count | the slot rule, 1172 of 1172 | container |
| Arena records sit in first write order, not arena order | `t7_gen_for`, arena 2 first | 1172 of 1172 with the reader accepting any order | container |
...
| The end time is a uint64 at trailer `+0`, in the file's time unit | correlation sweep over 33 cases | 1172 of 1172, 20 ns to 70010 ns; 100 for 100 ns in `t21_v_ts_1ns_1ns` | container |
| The marker offset is at trailer `+0x38` | `t5_tr1000`, where the marker moved | 1172 of 1172 | container |
| The marker is a list of `[first][last]` object index ranges, as many as trailer `+0x30` counts, covering exactly the objects with records | `t9_port_rec` held two entries where `t6_var_int` had shown one; `t9_mark_gap` put an unlogged object first | 1172 of 1172, the reader checks every object against the ranges | container |
...
| Each directory entry follows the section it describes | `t2_flat3`: `WDB.Event` at `0xe0+0x48`, RTTI and DBG the same | 1172 of 1172 | container |
| The page directory starts 48 bytes after the DBG entry | `t2_flat3`, reading the offsets | 1172 of 1172 | container |
...
| A page is a zlib stream that inflates to 10240 bytes | entropy profile, then `zlib` on `t1_bit_one_edge` | 1172 of 1172 | values |
...
| A signal has one record at time 0 and one per change, when logged from the start | `t0_bit_const` | `t3_late`, 1172 of 1172; `t45_log_late____` for a log started at 10 ns | values |
| The type table starts with `Xilinx ISim TYPE FILE 001` | `strings` on `t1_bit_one_edge` | 1172 of 1172 | types |
| `+32` of the type table is the number of types | correlation sweep | 1172 of 1172 | types |
| Type entries are `[len][tag]` name body | `t2_enum` against `t1_bit_one_edge` | 1172 of 1172 | types |
| Enumerations list their literals; `character` has 261 | `t2_enum`, `t2_character` | `truth.json` names | types |
| Integer entries hold the bounds, reals the bounds as float64 | `t2_integer`, `t2_real` | 1172 of 1172 | types |
| Physical entries list units with scales | `t2_time` | | types |
| Arrays hold element, index type and constraint triples | `t1_vec8` against `t2_array2d` | `t5_int_arr` | types |
...
| The DBG section starts with `Xilinx ISim DBG 006` and 18 region offsets | `t1_hier1` against `t2_hier3` | 1172 of 1172 | hierarchy |
| Scope records: name, parent, children, first object, unit, file, line | `t2_hier3` | 1172 of 1172 | hierarchy |
| Unit records: entity, architecture, kind, declaration count, file, line | `t2_hier3` | `t4_gen_diff_two` | hierarchy |
| Declaration records: name, file, line, size, type, ranges, kind | `t2_flat3` | 1172 of 1172 | hierarchy |
| Declaration word 1 is the index of the region 17 entry holding the value class of the declaration's objects | `t31_sv_w1_swap` against `t31_sv_w1_i5`, where swapping `int i = 5` and `logic s` swapped the words `1`, `0` to `0`, `1` and the entries `[1 0 0] [3 0 0]` to `[3 0 0] [1 0 0]` | `t12_v_params`, six words `0 1 2 1 0 1` over three entries; the reader's range check in 1172 of 1172; `t31_sv_w1_i0` through `t31_sv_w1_own50`, where the value and the writes leave the word alone | hierarchy |
...
| Declaration word 9 is the port mode: 0 inout, 1 in, 2 out, 3 buffer, 4 linkage, 5 none | `t8_port_in`, `t8_port_out`, `t8_port_inout`, `t8_port_buffer`, `t9_port_lnk` | 1172 of 1172 against the `port` field in `truth.json` | hierarchy |
...
| The file table holds compile and local paths | `t2_slv8` against `t1_vec8` | 1172 of 1172 | hierarchy |
| Regions 14 and 15 are executable statement lines per file | `t6_proc2` | `t2_hier3` | hierarchy |
| Instance records: handle, second handle, scope, kind, declaration | `t2_flat3` | 1172 of 1172 | hierarchy |
...
| The DBG word after the timestamp is the power of ten of the time unit, and every time in the file counts that unit | `t21_v_ts_1ns_1ns` against `t11_v_bit_edge`, `-9` and a change at 50 | `t21_v_ts_1ps_1ps`, `t21_v_ts_10ns`, `t21_v_ts_1ns_100` at `-10`, `t21_v_ts_1ps_1fs` at `-15`; the VCD `$timescale` agrees in 1172 of 1172 | hierarchy, values |
...
| DBG region 17 holds one 3 word entry per distinct value class among the objects, header word 13 counts them, and the region is padded to 8 bytes | `t25_sv_two_class` against `t25_sv_two_same`, 24 bytes and word 13 `2` for 16 and `1` | the reader's length check, 1172 of 1172 | hierarchy |
...
| The integral types are class 3 and `time` class 4 whatever the initializer; a packed type is classed by its initializer: 1 for a sized or fill literal or an expression of them, 3 or 4 for an unsized literal by the target's signedness, 6 for a string literal, 0 for none | `t27_sv_sgn8_pos` against `t27_sv_v8_neg`; `t27_sv_int_uns` against `t27_sv_byte_uns` | the tier 26 and 27 sweeps, 56 cases | hierarchy |
...
| The word before an array entry's triples counts them | `t11_v_time`, one constrained triple under a `1` | 1172 of 1172 | types |
...
| An alias entry holds a range count and triples; a `typedef` of a vector holds the vector's bounds there and the declaration holds none | `t12_sv_typedef` against `t11_v_vec8` | `t11_sv_struct`, `t11_sv_enum`, whose aliases count 0 | types |
...
| A `typedef` of an unpacked array holds every range in the alias, the unpacked one first, and the declaration none | `t13_sv_tdef_ua` against `t12_sv_unp2d` | `t12_sv_typedef` | types |
...
| A typedef of an unpacked struct array is a second alias over the array entry holding the range the declaration drops | `t35_sv_ust_tdef_` against `t35_sv_ust_arr__` | `t35_sv_pst_tdef_`, `t13_sv_tdef_ua` | types |
...
| An unconstrained two dimensional array type holds one `(0, 0, -2)` triple per dimension and the declaration holds the ranges | `t20_arr_2d_uncon` against `t18_arr_2dim` | `t20_rec_2dim`, two triples on a record field of a constrained two dimensional type | types |
...
| A scope that declares no data object adds `0x30` of handle space as a block and `0x90` as a process; an instance adds `0x20` more than a generate iteration of the same body once there are two of them, and every copy past the second adds `0x220` either way | `t78_scp_block___` against `t78_scp_base____`, `0x1200` against `0x11d0`, and `t78_scp_solo2___` against `t78_scp_gen2____`, `0x1670` against `0x1650` | `t78_scp_proc2___`, `t78_scp_proc3___`, `t78_scp_solo1___`, `t78_scp_solo3___`, `t78_scp_gen1____`, `t78_scp_gen3____` | hierarchy |
| A subprogram frame handle is an offset in that subprogram's own numbering and not a place in the handle space: frames start at `0xd0`, two subprograms' frames lie on top of each other, and a frame of 512 integers reaches `0x8d0`, past the first signal's `0x768`, while the signal and the handle space stay where they are | `t79_frm_512int__` against `t79_frm_001int__`, both with `s` at `0x768` and `0x11d0` of handle space | `t79_frm_016int__`, `t79_frm_064int__`, `t79_frm_256int__`, `t79_frm_two_64__` | hierarchy |
| The bytes of a subprogram's static composite value lie past every object, at the end of the handle space: adding one moves neither the signals nor the generic and process variable that follow them | `t80_stv_arr8____` against `t80_stv_none____`, `0x11f8` against `0x11d8` with `s` at `0x768`, `k` at `0xde0` and `a` at `0xde4` in both | `t80_stv_arr4____`, `t80_stv_rec8____`, `t80_stv_two_arr_` | hierarchy |
| A package of the design leaves `0x50` of handle space past the second region whatever it declares, and its block between the signals and the later objects is `0x28` plus its object bytes rounded up to 8 | `t81_pkt_fn______` and `t81_pkt_arr16___` against `t81_pkt_none____`, both leaving `0x50` past the region while the block goes from `0x28` to `0x78` | `t81_pkt_1con____`, `t81_pkt_4con____`, `t81_pkt_two_pk__`, which leaves `0xa0` | hierarchy |
| A constant of a package body is an object of the package like one of its header, and lies in the package's block; the static values of the package's own subprograms lie past the second region instead | `t82_pkb_1con____` and `t82_pkb_fn_stat_` against `t82_pkb_none____`, the first moving `k` from `0xe08` to `0xe10` and the second leaving it at `0xe08` while the total grows `0x10` | `t82_pkb_4con____`, `t82_pkb_arr16___`, `t82_pkb_deferred` | hierarchy |
| The 4 bytes a static value with a record in it adds over its declared size are added once for the value and not once per record: a record in a record, two of them, an array in a record and an array of four records all add the declared size and one 4 | `t83_rec_arr4rec_` against `t83_rec_1int____`, 32 declared bytes and `0x24`, against 8 and `0xc` | `t83_rec_2nested_`, `t83_rec_nested__`, `t83_rec_arr4____`, `t83_rec_arr_of__`, `t83_rec_4int____`, `t83_rec_8int____`, `t83_rec_1sul____` | hierarchy |
...
| At every time the VCD lists a value, the last database value at that time spells it, for every VCD variable of every case; the changes of value in the VCD are the changes of value in the database, and the VCD keeps a few of the database's writes of the value held and drops the rest | `TestVCD`, `bazel test //pkg/wdb:wdb_test --test_filter=TestVCD` | 1172 of 1172, `//hdl/counter:sim`, `//hdl/uart:sim` and `//hdl/serv:sim`; one real field of `t11_sv_struct_r` and every untyped time parameter excepted, where the VCD writes the `float64` bytes as a vector; the times were equal in every case through tier 35 and differ from tier 36 on | vcd |
| Two VCD variables with one identifier code are two objects with one handle and one offset | `TestVCD` on `t12_v_port_wire` and `t13_sv_iface` | 1172 of 1172 | vcd |
| The VCD leaves out every VHDL generic, constant and non bit type, every signal outside `tb`, and every Verilog unpacked array not named by a typedef, and nothing else | `TestVCD`, the `vcdOmitted` rule | 1172 of 1172 | vcd |
...
| An array type whose element is unconstrained writes its own index as `(0, 0, -2)` even when the index is written with bounds; the declaration holds the index and the element bounds | `t42_arr_unc_elem` against `t2_array2d______`, `array (0 to 1) of std_ulogic_vector` against `array (0 to 3) of std_ulogic_vector(7 downto 0)` | `t42_arr_unc_both`, which differs only in the index word; `t42_rec_unc_arr_` | types |
...
| A use clause adds handle space by the package, `0x604` for `std_logic_1164`, `0x1f8` for `numeric_std`, `0x400` for `math_real`, and adds the package and its body to the file table; the type of the signal adds nothing; a package of the design adds `0x80` plus the rounded storage of its constants, and its types and subprograms are free | `t47_use_numstd__` against `t1_bit_one_edge_`, the clause alone reproducing the `0x1f8` of the nine tier 2 cases that use it | `t47_use_1164_bit`, `t47_use_none____`, `t47_use_lib_only`, `t47_use_one_name`, `t47_use_textio__`, `t47_use_numbit__`, `t47_use_mathrl__`; `t47_use_pkg_emp_`, `t47_use_pkg_typ_`, `t47_use_pkg_4arr`, `t47_use_pkg_fn2_`, `t47_use_pkg_pr2_`, `t47_use_pkg_two_`, `t47_use_pkg_nul_` | container, hierarchy |
...
| In the second handle region an object's stride is its value size, the next object on a multiple of its own size, and an array or string adds 16 bytes over its elements; a constant and a generic stride as a variable | `t52_var_real____`, `t52_var_vec8____`, `t52_var_arr4____` against `t52_var_int_____`, 8, `0x18` and `0x20` for 4 | the other twelve `t52_var_`, `t52_con_` and `t52_gen_` cases; `t9_gen_types____` | hierarchy |
...
| A VHDL port under a Verilog net that holds a VHDL value from elaboration holds the value and no `U`; a VHDL signal driven by a Verilog `assign` holds `U`, the `X` of the assign, then the value | `t49_mix_deep____` against `t49_mix_2port___` and `t21_mix_vh_in_v_` | | values |
| A constant declared in a subprogram is a local of kind `0x14` and class 3 or 4 as a variable is, and the frame gives it its size plus 16 bytes where a scalar variable gets its size | `t55_sub_con_loc_` against `t55_sub_loop____`, `v` on `0x58` for `0x44` | `t55_sub_con_nori`, `t55_sub_2con____`, `t55_sub_con_real`, `t55_sub_var_init`, `t55_sub_con_arr_` | hierarchy |
...
| A subprogram's static composite values add handle space by their bytes: a composite local's literal or default initial value, and every aggregate or string literal in the body or the call; a type, a scalar local, and a value computed at the call take none | `t56_typ_arr_loc_` against `t56_typ_arr_unus`, `0x10` for four integers; `t56_typ_arr_lit_` against `t56_typ_arr_noin`, `0x10` more for an aggregate in the body; `t56_sub_arr_dyni`, `t56_typ_rec_prm_`, nothing for a computed initial value | `t56_typ_arr8_loc`, `t56_typ_vec4_loc`, `t56_typ_vec4_2lc`, `t56_typ_vec_noin`, `t56_typ_rec_loc_`, `t56_sub_rec_3int`, `t50_sub_func_prm`, `t50_sub_str_loc_` | hierarchy |
| A record's static value adds its declared size plus 4, and a record declares a multiple of 8 bytes, 8 for one integer | `t56_sub_rec_1int`, `t56_sub_rec_2int` against `t56_sub_rec_3int`, `t56_sub_rec_4int`, `0xc` then `0x14` | `t56_sub_rec_2rl_`, `t56_typ_rec_arr_` | hierarchy |
...
| A SystemVerilog enumeration declares its width in the range its own entry holds after the literals; the base type of `enum logic [1:0]` is a plain `logic` and holds none | `t67_esz_pk_2bit_` against `t11_sv_struct___`, a 4 bit struct whose VCD reads `1011` | `t67_esz_pk_4bit_`, `t67_esz_pk_int__`, and `//hdl/ibex:sim`, whose `csr_op_e` is 2 bits and `x_debug_ver_e` 4 in one file | types |
...
| `t9_pkg_sig` against `t1_bit_one_edge` | a signal in a package | the package signal is on the first handle and is not logged; arena 0 unwritten |
...
| `t11_sv_struct3`, `t11_sv_struct40`, `t11_sv_struct_r` against `t11_sv_ustruct` | three fields, a 40 bit field, a `real` field | a field fills the pairs a standalone value would, low word first |
...
| `t12_sv_typedef` against `t11_v_vec8` | `typedef logic [7:0] byte_t` | the alias entry holds the bounds; the declaration holds none |
...
| `t13_sv_tdef_ua` against `t12_sv_unp2d` | a typedef of an unpacked array | the alias holds both ranges; the declaration holds none |
...
| `t47_use_numstd__` against `t1_bit_one_edge_` | `use ieee.numeric_std.all`, unused | `0x1f8` of handle space and two file table entries; the tier 2 "cost of a type" was the clause |
...
| `t49_sub_var_prm_` and `t49_sub_vec_prm_` against `t23_sub_sizes___` | a `variable` class scalar parameter; a `constant` class vector parameter | 3 and 4 follow the type, not the class or the mode |
...
| `t59_frc_v_const_`, `t59_frc_v_bit___` against `t59_frc_none____` | the vector forced whole, and one bit of it | `1111` at 0, 10 and 20 ns; `1000`, `1172`, `1010` once each |
...
| `t60_dbg_class_2_`, `t60_dbg_class_d_` against `t60_dbg_class___` | a second `logic [3:0]` field; a parent class | the field holds its range triple and a `0`, the unnamed vector entry closes with `1` and `int` with `2`; the parent comes first with id `1`, the derived class names it by index and lists its own field only |
...
| `t61_num_a_then_q` against `t61_num_q_then_a` | an associative array and a queue, declared in either order | the numbers follow the declaration order, `2` and `3` against `1` and `3`; the associative array is numbered twice |
...
| `t67_esz_pk_int__` against `t67_esz_pk_2bit_` | the enumeration over `int` | 32 bits from the base type, which holds the range in that case |
...
| `t78_scp_block___`, `t78_scp_proc2___` against `t78_scp_base____` | an object-less block and an object-less process | `0x30` for the block, `0x90` for the process |
| `t78_scp_solo1___`, `t78_scp_solo2___`, `t78_scp_solo3___` against `t78_scp_gen1____`, `t78_scp_gen2____`, `t78_scp_gen3____` | instances of a portless child against generate iterations of the same body | `0x30` more at one copy, `0x20` more at two and three |
| `t79_frm_512int__`, `t79_frm_256int__` against `t79_frm_001int__` | a frame of 512 and of 256 integers instead of one | the frame reaches `0x8d0` and `0x4d0`; the signal and the handle space do not move |
| `t79_frm_two_64__` against `t79_frm_064int__` | a second procedure with its own 64 variables | both frames run `0xd0` to `0x1d0`: they lie on top of each other |
| `t80_stv_arr4____`, `t80_stv_arr8____`, `t80_stv_rec8____` against `t80_stv_none____` | a static composite value in the function | `0x10`, `0x20` and `0xc` more handle space, and no handle moved |
| `t81_pkt_fn______`, `t81_pkt_1con____`, `t81_pkt_arr16___` against `t81_pkt_none____` | a package of the design, declaring nothing, one constant, or an array | the generic moves by `0x28`, `0x30` and `0x78`; the total moves by `0x50` more in each |
| `t82_pkb_1con____`, `t82_pkb_arr16___` against `t82_pkb_none____` | constants in the package body rather than the header | the block grows as it does for a header constant |
| `t82_pkb_fn_stat_` against `t82_pkb_none____` | a static composite value in the package's own function | `0x10` past the second region, and the block unchanged |
| `t83_rec_arr4rec_`, `t83_rec_2nested_` against `t83_rec_1int____` | four records in an array, and two records in one | the declared size and one 4, so the 4 is per value |
...
1. Trailer `+0x18` is the handle space size, which fixes the slot count.
   ...
   what a library package adds by, `0x604` for `std_logic_1164`,
...
6. Handles of generics, constants, variables and loop indexes come
   ...
   the strides are the value sizes and the scope figures of tiers 52
   ...
   What the `0x738` before the first signal holds is open.
   What a package leaves past the second region is `0x50`, flat, for
   a package of the design, tier 81: a package with one function, one
   constant, four constants or a constant array of sixteen integers
   leaves the same `0x50`, and two packages leave `0xa0`, while the
   block between the signals and the later objects is `0x28` plus the
   object bytes rounded up to 8.
   The library figures, `0x78` for `textio`, `0x70` for `env`,
   `0x15c` for `std_logic_1164`, `0x100` for `numeric_std` and `0xf8`
   for `math_real`, are that `0x50` and something else each.
   Tier 82 names the something else: the static values of the
   package's own subprograms land there, `0x10` for a function with
   an array local of a literal value, while a constant of the body or
   a deferred constant lies in the block like a constant of the
   header.
   What is left of the question is the count of such literals in each
   library body.
   Tier 79 rules out one reading of the first: a subprogram's frame
   objects are numbered from `0xd0` upward in the subprogram's own
   numbering, and a frame of 512 integers reaches `0x8d0`, past the
   `0x768` of the first signal, without moving that signal or the
   handle space.
   So the space below the first signal is not room set aside for
   frames.
   Tier 78 measures the other two.
   A scope that declares no data object adds `0x30` when it is a
   block and `0x90` when it is a process, so the block is the `0x28`
   of a scope rounded up to 8 and the process adds `0x60` more for
   its own frame.
   Against an instance of the same body, a generate saves `0x30` at
   one copy and `0x20` at two and at three, and past the second copy
   each further copy adds `0x220` either way, so the saving sits in
   the fixed part and is not a cost per copy.
...
13. The handle space costs of a subprogram, 8 bytes, and of a `signal`
    ...
    further one, where the frame gives a file parameter 8 and an alias nothing.
    The handle space a subprogram's static composite values add,
    tier 56, is placed by tier 80: the bytes lie past every object, at
    the end of the handle space, since adding them moves neither the
    signals nor the generic and process variable that follow them.
    Why a record adds 4 over its declared size is open; tier 83 adds
    that the 4 goes with the value once and not with each record in
    it, since a record inside a record, two of them, an array inside a
    record and an array of four records all add the declared size and
    one 4.
...
19. The value class codes of region 17 are 0, 1, 3, 4 and 6 in the
    ...
    `real` and `realtime` 0, and a packed type is classed 0, 1, 3, 4 or 6
    by its initializer as converted to the target: none, a real, a
...
24. What the numbering under `-debug all` is for is open.
    ...
    array is one of those descriptors and holds its number in its
    ...
    holds, and a dynamic array and a queue leave none.
